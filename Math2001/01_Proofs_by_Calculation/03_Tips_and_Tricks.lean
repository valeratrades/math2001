...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z^4 - z^3 - z^2 + 2*z + 1 = (z^2 - 2)^2 +4*z^2 - 4 - (z^2 - 2)*z -2*z - (z^2 - 2) - 2 + 2*z + 1 := by ring
    _ = (0)^2 +4*z^2 - 4 - (0)*z -2*z - (0) - 2 + 2*z + 1 := by rw[h1]
    _ = 4*(z^2 - 2) + 8 - 5 := by ring
    _ = 4*0 + 8 - 5 := by rw[h1]
    _ = 3 := by ring
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    t ^ 4 + 3 * t ^ 3 - 3 * t ^ 2 - 2 * t - 2 = (t^2 - 4)^2 +8*t^2 -16 + 3 * (t ^ 2 - 4)*t +12*t - 3 * (t ^ 2 - 4) -12 - 2 * t - 2 := by ring
    _ = (0)^2 +8*t^2 -16 + 3 * (0)*t +12*t - 3 * (0) -12 - 2 * t - 2 := by rw[ht]
    _ = 8*(t^2 -4) + 10*t + 2 := by ring
    _ = 8 * 0 + 10*t + 2 := by rw[ht]
    _ = 10 * t + 2 := by ring


-- fuck me, this one is difficult
example {x y : ℝ} (h1 : x + 3 = 5) (h2 : 2 * x - y * x = 0) : y = 2 :=
  calc
    y = ((2 * x - y * x) - (2 * (x + 3) - y * (x + 3) + 2*y - 6) - 3*y) / -2 := by ring
    _ = (0 - (2 * 5 - y * 5 + 2*y - 6) - 3*y) / -2 := by rw[h1, h2]
    _ = 2 := by ring
...
example {p q r : ℚ} (h1 : p + q + r = 0) (h2 : p * q + p * r + q * r = 2) :
    p ^ 2 + q ^ 2 + r ^ 2 = -4 :=
  calc
    -- p^2 + p*q + p*r + q*p + q^2 + q*r + r*p + r*q + r^2
    p ^ 2 + q ^ 2 + r ^ 2 = (p + q + r)^2 - ((p*q + p*r + q*r) + (p*q + p*r + q*r)) := by ring
